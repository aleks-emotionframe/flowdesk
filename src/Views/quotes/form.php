...
$pageTitle = $isEdit ? 'Offerte bearbeiten' : 'Neue Offerte';
$action = $isEdit ? '/quotes/' . $quote['id'] . '/update' : '/quotes';

ob_start();
?>

<div class="max-w-4xl">
    <a href="<?= $isEdit ? '/quotes/' . $quote['id'] : '/quotes' ?>" class="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 mb-6">
    ...
    <form method="POST" action="<?= $action ?>" id="quote-form" class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="contact_id" name="contact_id" required
                        ...
                        <?php foreach ($contacts as $c): ?>
                            <option value="<?= $c['id'] ?>" <?= (int)($quote['contact_id'] ?? 0) === (int)$c['id'] ? 'selected' : '' ?>>
                                <?= e(contactName($c)) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" value="<?= e($quote['title'] ?? '') ?>" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                           placeholder="z.B. Website Redesign">
                </div>
                <div>
                    <label for="valid_until" class="block text-sm font-medium text-gray-700 mb-1">Gültig bis</label>
                    <input type="date" id="valid_until" name="valid_until" value="<?= e($quote['valid_until'] ?? '') ?>"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                </div>
                <div>
                    <label for="mwst_satz" class="block text-sm font-medium text-gray-700 mb-1">MwSt-Satz (%)</label>
                    <input type="number" id="mwst_satz" name="mwst_satz" value="<?= e((string)($quote['mwst_satz'] ?? '8.1')) ?>" step="0.1"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                           onchange="recalc()">
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <textarea name="notes" rows="3"
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                      placeholder="Optionale Bemerkungen..."><?= e($quote['notes'] ?? '') ?></textarea>
        </div>

        <div class="flex items-center justify-end space-x-3">
            <a href="<?= $isEdit ? '/quotes/' . $quote['id'] : '/quotes' ?>" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Abbrechen</a>
            <button type="submit" class="px-6 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 transition-colors">
                <?= $isEdit ? 'Speichern' : 'Offerte erstellen' ?>
            </button>
        </div>
    </form>
</div>
